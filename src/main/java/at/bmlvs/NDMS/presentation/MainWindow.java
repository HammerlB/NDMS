...
public class MainWindow
{
	...
	public void addTopMenu()
	{
		...
		mainFrame.setIconImage(new ImageIcon("icons/ndms.png").getImage());
		...
		JMenuItem importe = new JMenuItem("Importieren", new ImageIcon(
				"icons/smallimport.png"));
		...
		JMenuItem export = new JMenuItem("Exportieren", new ImageIcon(
		...
		// Konfigmenü
		JMenuItem portkonf = new JMenuItem("Port-Konfiguration", new ImageIcon(
				"icons/smallportkonf.png"));
		JMenuItem vorlagen = new JMenuItem("Vorlagen", new ImageIcon(
				"icons/smallvorlagen.png"));
		...
		// Werkzeuge
		JMenuItem tostandart = new JMenuItem(
				"Zur Standartkonfiguration zurücksetzten", new ImageIcon(
						"icons/smallresetstandart.png"));
		JMenuItem tomanage = new JMenuItem(
				"Zur Managementkonfiguration zurücksetzten", new ImageIcon(
						"icons/smallresetmanag.png"));
		JMenuItem diagnose = new JMenuItem("Diagnose", new ImageIcon(
				"icons/smalldiag.png"));
		...
		// Hilfe
		JMenuItem handbook = new JMenuItem("Benutzerhandbuch", new ImageIcon(
				"icons/smallbook.png"));
		JMenuItem about = new JMenuItem("Über NDSM", new ImageIcon(
				"icons/smallabout.png"));
	...
	private void save()
	{
		 ...
		 * f.setAcceptAllFileFilterUsed(false); f.setFileFilter(new FileFilter()
		 * { public String getDescription() { return "XML-Files(*.xml)"; }
		 * 
		 * // shows only directories and xml-files in the file chooser public
		 * boolean accept(File f) { if (f.isDirectory()) return true; return
		 * f.getName().toLowerCase().endsWith(".xml") || f.isDirectory(); } });
		 * 
		 * int returnValue = f.showSaveDialog(M.this); if (returnValue ==
		 * JFileChooser.APPROVE_OPTION) { // Gets the selected file and attaches
		 * the file extension File fileToSave = f.getSelectedFile(); String fa =
		 * fileToSave.getPath(); if (!fa.toLowerCase().endsWith(".xml")) { fa +=
		 * ".xml"; }
		 * 
		 * for (TemplatesToPathLinker m :
		 * ServiceFactory.getPersistenceService().getTemplates()) { if
		 * (m.getElement().equals(
		 * ServiceFactory.getModelService().getActiveModel())) { m.setPath(fa);
		 * } }
		 * 
	...
	public void load()
	{
		...
		{
			// Templates templates =
			// ServiceFactory.getPersistenceService().loadTemplate(file.getAbsolutePath()).getElement();
		}
	}
}
